...
class SQLCompiler(compiler.SQLCompiler):
    ...
    def as_sql(self, with_limits=True, with_col_aliases=False):
        ...
        if self.query.distinct:
            if ((list(self.connection.settings_dict.keys())).__contains__('FETCH_DISTINCT_ON_TEXT')) and not \
        ...
        if not ( with_limits and ( self.query.high_mark is not None or self.query.low_mark ) ):
            ...
            if self.query.low_mark is 0:
                return sql_ori.replace("SELECT","SELECT FIRST %s" % self.query.high_mark), params
